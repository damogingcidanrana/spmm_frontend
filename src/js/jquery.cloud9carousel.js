...
;(function($) {
  ...
  var Item = function( element, options ) {
    ...
    if( transform && options.transforms )
      this.element.style[transform + "Origin"] = "0 150px";
  ...
  var Carousel = function( element, options ) {
    ...
    this.go = function( count ) {
      ...
      this.items.forEach(function(element,index){
        if (element.scale>maxscale) {
          maxscale = element.scale;
          if (count>0) {
            if (index==0) {
              behind = self.items.length-1;
            } else {
              behind = index-1;
            }
          } else {
      ...
      $(this.wrapper).addClass('transform');
      setTimeout(function(){
        $(self.wrapper)[0].className = self.wrapper.replace('.','');
        $(self.wrapper).removeClass('transform');
        $(self.wrapper).addClass($(self.items[behind].element).attr('data-wrapper'));
      },500);
      $(this.items[behind].element).addClass('pointer-events').siblings().removeClass('pointer-events');
      //alasvet end
    ...
    this.finishInit = function() {
      //alasvet start
      $(this.wrapper).addClass($(items[0]).attr('data-wrapper'));
      $(items[0]).addClass('pointer-events').siblings().removeClass('pointer-events');;
      //alasvet end
      //
